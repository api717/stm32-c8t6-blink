...
{
    while (I2C_GetFlagStatus(IIC_NUM, I2C_FLAG_BUSY));//检查总线是否忙
    ...
    while (!I2C_CheckEvent(IIC_NUM, I2C_EVENT_MASTER_TRANSMITTER_MODE_SELECTED));//传输模式 EV6
    I2C_SendData(IIC_NUM, addr);  /* 配置数据写入地址 */
    while (!I2C_CheckEvent(IIC_NUM, I2C_EVENT_MASTER_BYTE_TRANSMITTED)); //EV8
...
#else
    IIC_Start();     		//发出 START（相当于硬件的 GenerateSTART + 等 EV5）
    IIC_SendByte(OLED_ID);  //发地址 + Write（相当于 Send7bitAddress）
    /* 等待应答 */
    while (IIC_Wait_ACK())  //等从机 ACK（相当于等 EV6）
        ;
    IIC_SendByte(OLED_WR_DATA);//发寄存器地址
    /* 等待应答 */
    while (IIC_Wait_ACK()) //等 ACK（相当于 EV8）
        ;
    IIC_SendByte(data);    //发数据
    /* 等待应答 */
    while (IIC_Wait_ACK())  //等 ACK（相当于又一个 EV8）
        ;
    IIC_Stop();
#endif
}
...
/**
 * @brief  OLED_ShowCN24x24  显示 24×24 点阵汉字
 * @param  x     起始列坐标 (0~127 - 24 = 103 最大合理值)
 * @param  y     起始页坐标 (0~7 - 3 = 4 最大合理值，建议 0~4)
 * @param  n     汉字在 F24x24[] 中的索引
 * @retval 无
 * @note   每个汉字占 72 字节，跨 3 页显示
 */
void OLED_ShowCN24x24(unsigned char x, unsigned char y, unsigned char n)
{
    unsigned char wm;               // 列计数器
    unsigned int adder = 72 * n;    // 每个汉字 72 字节

    // 第一页（上 8 行）
    OLED_SetPos(x, y);
    for (wm = 0; wm < 24; wm++)
    {
        Oled_Write_Data(F24x24[adder++]);
    }
...
void OLED_DrawBMP_Pro(unsigned char x0, unsigned char y0, unsigned char width, unsigned char height, const unsigned char *BMP)
{
    unsigned char page, col;
    unsigned int idx = 0;

    // 计算页数
    unsigned char pages = (height + 7) / 8;  // 向上取整

    for (page = 0; page < pages; page++)
    {
        OLED_SetPos(x0, y0 + page);  // 设置页地址（y0是起始页）

        for (col = 0; col < width; col++)  // 每页写 width 个字节（宽度像素对应width字节）
        {
